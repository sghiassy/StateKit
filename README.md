...
### Benefits
  ...
  * **Calm your code** - With a proper state chart managing flow control, functions can now begin to calm down. Their code, which used to brace for being called inappropriartly or had to check to figure out what had happened previously, can now dispense with those worries. Functions can now safely assume (given the right state chart structure) that they are be called only when actually needed and can safely assume any necessary object allocations or actions have been taken care of a parent application's state.
